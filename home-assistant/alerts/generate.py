...
def source_automations(name, mapping, freshness):
    ...
    if name in {"aranet", "nest"}:
        # These integrations report state changes, not periodic heartbeats.
        # Their entity availability is authoritative; timestamp aging would
        # incorrectly mark stable humidity, battery, and climate values stale.
        ...
        if name == "aranet":
            unavailable = f"{{% set obj = {lookup} %}}{{{{ not obj or obj.state in ['unknown','unavailable'] }}}}"
            current = f"{{% set obj = {lookup} %}}{{{{ obj and obj.state not in ['unknown','unavailable'] }}}}"
    else:
