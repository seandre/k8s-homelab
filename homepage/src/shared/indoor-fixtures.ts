...
const sourceFor = (alias: IndoorEntityAlias) => alias.startsWith('aranet_') ? 'ARANET_LOCAL' as const : alias.startsWith('nest_') ? 'NEST_CLOUD' as const : 'COWAY_CLOUD' as const;
const reading = (alias: IndoorEntityAlias, value: number | null, unit: '°F' | '%' | 'hPa' | 'ppm' | 'µg/m³', state: FixtureState = 'healthy') => ({
...
const option = (options: string[], dependency: 'NEST_CLOUD' | 'COWAY_CLOUD', supported = true) => ({ supported, options: supported ? options : [], dependency });
...
export function indoorFixture(state: FixtureState = 'healthy'): IndoorState {
  ...
  return {
    rooms: [
      { alias: 'living_room', name: 'Living Room', temperatureF: nestState === 'healthy' ? 70 : aranetState === 'healthy' || aranetState === 'stale' ? 69.8 : null, humidityPercent: aranetState === 'healthy' || aranetState === 'stale' ? 43 : null, co2Ppm: aranetState === 'healthy' || aranetState === 'stale' ? 612 : null, pm25WorstMicrogramsM3: living.readings.pm25.value, activeAlertCount: 0, freshness: state === 'healthy' ? 'CURRENT' : state === 'stale' || state === 'partial' ? 'STALE' : state === 'unsupported' ? 'NOT_SUPPORTED' : 'UNAVAILABLE' },
