...
describe('indoor action gateway', () => {
  ...
  it('continues ventilation on available devices when Nest fan control fails', async () => {
    ...
    const executor = {
      execute: vi.fn(async (command: IndoorCommand) => {
        calls.push(command);
        if (command.type === 'NEST_SET_FAN_TIMER') throw new Error('Nest OAuth unavailable');
        if (command.type === 'COWAY_SET_PRESET') {
          const target = state.indoor.purifiers.find((purifier) => purifier.alias === command.target)!;
          target.power = true;
          target.preset = command.preset;
          target.speed = command.preset === 'RAPID' ? 3 : 1;
        }
      }),
    };
    const gateway = new IndoorActionGateway(
    ...
    await expect(gateway.accept({
      idempotencyKey: '5f576a87-7459-46ca-a144-b55a42111210',
