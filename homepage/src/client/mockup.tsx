      ...
      <main className="dashboard" id="overview">
        ...
        <div className="pve-overview">
          ...
          <div className="overview-summary-grid">
            ...
            <Panel className="overview-summary-card" title="OKD" eyebrow="WORKLOAD / OKD" severity={okdCluster.metadata.severity} freshness={okdCluster.metadata.freshness} href="/okd"><div className="metric-grid"><Metric label="HEALTH" value={okdCluster.metadata.severity} /><Metric label="NODES" value={`${okdCluster.readyNodeCount ?? '—'} / ${okdCluster.nodeCount ?? '—'}`} /><Metric label="CPU" value={okdCluster.cpuUsedCores === null || okdCluster.cpuCapacityCores === null ? '—' : Math.round(okdCluster.cpuUsedCores / okdCluster.cpuCapacityCores * 100)} unit="%" /><Metric label="MEMORY" value={okdCluster.memoryUsedBytes === null || okdCluster.memoryCapacityBytes === null ? '—' : Math.round(okdCluster.memoryUsedBytes / okdCluster.memoryCapacityBytes * 100)} unit="%" /><Metric label="WORKLOADS" value={okdCluster.workloadCount ?? '—'} /></div></Panel>
