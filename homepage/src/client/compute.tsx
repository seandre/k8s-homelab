...
export function ComputeScreen({ bootstrap = healthyBootstrapFixture }: { bootstrap?: Bootstrap }) {
  ...
  return (
    <main className="dashboard" id="compute">
      ...
      <section className="compute-section" aria-labelledby="okd-nodes-title"><div className="section-heading"><span className="panel-eyebrow">WORKLOAD / OKD</span><h2 id="okd-nodes-title">OKD control-plane nodes</h2></div>{model.okdNodes.length ? <div className="pve-overview compute-host-grid">{model.okdNodes.map((node) => <OkdNodePanel key={node.id} node={node} timeSeries={bootstrap.timeSeries} expanded={expandedHosts.includes(node.id)} onExpand={() => toggleExpandedHost(node.id)} />)}</div> : <div className="empty-state">No successful OKD node sample is available.</div>}</section>
    </main>
  );
}
