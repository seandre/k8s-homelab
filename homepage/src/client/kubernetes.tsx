...
export function OkdScreen({ bootstrap = healthyBootstrapFixture }: { bootstrap?: Bootstrap }) {
  ...
  return <main className="dashboard" id="okd">
    ...
    <section className="compute-section" aria-labelledby="okd-node-health"><div className="section-heading"><span className="panel-eyebrow">NODE HEALTH</span><h2 id="okd-node-health">Schedulable control plane</h2></div><div className="pve-overview compute-host-grid">{model.okdNodes.map((node) => <OkdNodePanel key={node.id} node={node} timeSeries={bootstrap.timeSeries} />)}</div></section>
