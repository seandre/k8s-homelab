...
export function OkdScreen({ bootstrap = healthyBootstrapFixture }: { bootstrap?: Bootstrap }) {
  ...
  return <main className="dashboard" id="okd">
    ...
    <section className="compute-section" aria-labelledby="okd-node-health"><div className="section-heading"><span className="panel-eyebrow">NODE HEALTH</span><h2 id="okd-node-health">Schedulable control plane</h2></div><div className="compute-node-grid">{model.okdNodes.map((node) => <Panel className="workload-box" key={node.id} title={node.name} eyebrow="CONTROL PLANE" severity={node.metadata.severity} freshness={node.metadata.freshness}><div className="metric-grid"><Metric label="CPU" value={node.cpuPercent ?? '—'} unit="%" /><Metric label="MEMORY" value={node.memoryPercent ?? '—'} unit="%" /><Metric label="STATUS" value={nodeState(node)} /></div><KubernetesNodeGraphs node={node} timeSeries={bootstrap.timeSeries} /></Panel>)}</div></section>
