...
function CapacityGraphs({ cluster, timeSeries }: { cluster: NonNullable<ReturnType<typeof buildOverviewModel>['k3s']>; timeSeries: TimeSeries[] }) {
  return <div className="k8s-graph-grid">
    <DotGraph label="CPU" values={seriesValues(timeSeries, 'k3s CPU', utilization(cluster.cpuUsedCores, cluster.cpuCapacityCores))} unit="%" tone="cpu" height={4} />
    <DotGraph label="MEMORY" values={seriesValues(timeSeries, 'k3s MEMORY', utilization(cluster.memoryUsedBytes, cluster.memoryCapacityBytes))} unit="%" tone="memory" height={4} />
  </div>;
}
