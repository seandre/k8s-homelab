...
| IE-006 Aranet4 | **OWNER GATE PENDING** | IE-005 complete; owner must verify firmware >=1.2.0 and Smart Home Integration | Credential-free alias contract and runbook are implemented; live discovery, all five local readings, freshness, and Atom-loss behavior remain to be proven without fabricated values. |
