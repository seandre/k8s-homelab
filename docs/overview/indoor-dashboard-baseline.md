...
| IE-002 Coway compatibility harness | READY | IE-001 | `0.6.1` is SHA/checksum-pinned; unchanged upstream imports and passes config-flow/entity tests on HA `2026.7.2`; exact 250S entities/services are recorded, or a minimal documented patch is tested and offered upstream. |
| IE-003 Production HA image | BLOCKED | IE-002 | Digest-pinned official base contains verified Coway source; CI checks HA config/import/tests/build, emits SBOM/provenance and scan results, and publishes immutable SHA tags to GHCR. |
| IE-004 HA k3s foundation | BLOCKED | IE-003 | Argo is Synced/Healthy; private onboarding works; the 10 GiB writable state survives replacement; prior-image rollback is proven; only initial ingress/DNS/external-HTTPS paths exist. |
| IE-005 AtomS3 Lite proxy | BLOCKED | IE-004 plus owner USB flash/secrets gate | Encrypted proxy is discovered, reboots/reconnects, and accepts OTA; only k3s nodes `.21-.23` can reach its reserved IoT address on TCP 6053; unrelated cross-VLAN tests stay blocked. |
| IE-006 Aranet4 | BLOCKED | IE-005 | Firmware >=1.2.0 and Smart Home Integration are verified; all five aliases update locally; freshness and Atom-loss behavior pass without fabricated values. |
| IE-007 Nest | BLOCKED | IE-004 plus owner OAuth/Device Access gate | Official integration reads and controls every advertised capability; cloud loss is unavailable, and unsupported features are absent. |
