...
| NWS + AirNow weather chain | Utility/weather owner | NWS point, station observation, and hourly forecast APIs; AirNow current observation API; Open-Meteo weather and air-quality fallback APIs; approved Portland `97209` coordinates: `45.527412, -122.686270` | HTTPS REST | NWS observed conditions with hourly-forecast fallback; AirNow U.S. AQI; Open-Meteo sunrise/sunset, PM2.5, PM10, and provider-outage fallback | `homepage-airnow-readonly` / `api-key` (optional; absence selects Open-Meteo AQI fallback) | Conditions 10m / 5s; AQI 30m / 5s; NWS discovery 24h | Retain the last successful value; conditions stale after 35m (accommodating NWS observation QC delay) and AQI after 75m; provider fallback is identified in source metadata | Send the required fixed NWS User-Agent; log only provider, operation, and status/error class; never log coordinates, URLs, query strings, keys, or response bodies | `IMPLEMENTED`: NWS observation → NWS forecast → Open-Meteo conditions and AirNow → Open-Meteo AQI fallback |
