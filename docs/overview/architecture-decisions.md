...
| 2026-07-19 | Replace the stock Homepage interface with a purpose-built, stateless, btop-inspired TypeScript application | A custom React/Fastify interface can preserve the existing links and utilities while presenting normalized Prometheus and product-API telemetry consistently across k3s and future OKD deployments |
| 2026-07-19 | Keep cross-cluster homepage switching manual until an independent redundant traffic and DNS design is tested | A second application copy alone cannot provide the requested 30-second recovery while `bastion-01`, internal DNS, or the active ingress remains a single point of failure |
| 2026-07-20 | Serve the custom Homepage through a separately named production Service while retaining the stock resources | The existing Ingress identity and TLS remain stable, selectors stay disjoint, and Git can restore the stock Service without rebuilding or deleting its ConfigMap |
| 2026-07-20 | Retain the stock Homepage as the named rollback target after v1 closeout | Removal requires a separate approved retention decision; the HP-030 drill proved Git-only rollback and forward recovery without changing the stock image or ConfigMap |
| 2026-07-20 | Keep OKD ownership, manual cross-cluster switching, and automatic failover outside v1 | OKD is not provisioned, and the current single HAProxy/DNS dependency cannot support a tested 30-second failover claim |
| 2026-07-27 | Optimize the Homepage workflow on GitHub-hosted runners before considering an isolated self-hosted runner | Parallel verification, one cached container build, exact-digest scanning, and automatic gated GitOps promotion remove known waste without adding runner patching, isolation, credential, availability, or recovery responsibilities |
